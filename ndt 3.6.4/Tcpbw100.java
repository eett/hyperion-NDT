...
/* 
 * In compliance with the license above, this is a prominent notice that this piece of source
 * code is a modified version of the web100 network diagnostic tool. 
 * 
 * Every possible care has been taken to ensure that the NDT's original functionality is
 * completely intact. This means that, to the best of our knowledge and ability, this source
 * remains compatible with the original NDT and can thus be used with the same web100srv with 
 * the original. 
 * 
 * To serialize the result, this software uses Douglas Crockford's JSON library, available at 
 * https://github.com/douglascrockford/JSON-java. The license for that library includes this 
 * restriction: "The software shall be used for good, not evil." 
 * 
 * Added functionality includes among other minor changes:
 * 1.A mechanism to report back the measurement result, upon serializing it into a JSON structure
 * 2.Aesthetic changes in the applet/application to provide a simplified interface. Original interface
 *   is still available through a "details" button. Localization for the simplified interface is 
 *   separate from the original NDT localization to avoid modifying the original NDT resource files. 
 *   The simplified interface is localized for Greek, English
 *   Lastly, a Greek localization for the original NDT is also included. 
 *   
 * Athanasios Douitsis (aduitsis at noc.ntua.gr) 2011
 * 
 * Institute of Communications and Computer Systems (www.iccs.gr) / 
 * Hellenic Telecommunications and Post Commission (www.eett.gr)
 * 
 */
...
// to serialize the measurement results in a JSON record we import
// the org.json library. See also: http://www.json.org/java/
import org.json.*;
...
/*
 ...
 //this method will blank the indicator labels (i.e. when the measurement starts)
 //padding is an attribute containing a number of spaces. See bellow. 
...
**/	
  ...
  // this set will be used to organize the result to be transmitted back to the 
  // server accepting the measurement reports. userId and connectionId are opaque
  // and will not be interpreted in any way by this applet/application
  ...
  //simplified interface progressbar
  JProgressBar simple_progressBar = new JProgressBar();

  //whitespace distance separating the key from the value
  //for each measurement component
  String padding = "     ";
  
  JLabel dnlLbl = new JLabel();
  JLabel uplLbl = new JLabel();
  JLabel rttLbl = new JLabel();
  JLabel lossLbl = new JLabel();
  JLabel jitterLbl = new JLabel();
 
  String measurement_result;
  String current_message;

  //this is the simplified interface localization resource
  ResourceBundle res;
  
  private boolean measurement_is_running = false;
  private int progress_completed = 0;

  public String get_measurement_result() {
	  return measurement_result; 
  }
  
  public boolean measurement_running() {
	  return measurement_is_running;
  }
  ...
  public String get_current_message() {
	  return simple_progressBar.getString();
  }
  ...
  class TestWorker implements Runnable
  {
    public void run()
    {
      if (!testInProgress) {
        ...
        //measurement starts - disable the start button, change the progress bar accordingly
		startTest2.setEnabled(false);
		measurement_is_running = true;
        startTest2.setText(res.getString("measuring"));
        simple_progressBar.setIndeterminate(false);
		simple_progressBar.setString(res.getString("starting"));
		simple_progressBar.setValue(0);
        ...
        startTest2.setEnabled(true);
        measurement_is_running = false;
        startTest2.setText(res.getString("start"));
        testInProgress = false;
		getContentPane().remove(sPanel);
        getContentPane().validate();
        getContentPane().repaint();
        
        //measurement finished, reset the progress bar
        simple_progressBar.setString(res.getString("completed"));
		...
		if (!failed) {
			...
			//results.append("Download speed was " + Double.toString(s2cspd) + "\n");
			//results.append("Submitting results to " + reportHost + "\n");
			//System.err.println("UserId is " + userId + "\n");
			report.put("measurement_tool", "ndt");
			report.put("version", "0.02 ntua testing prototype");
			report.put("is_application", new Boolean(isApplication).toString());
			report.put("user_id", userId);
			report.put("connection_id", connectionId);
			report.put("report_host", reportHost);
			report.put("report_port", Integer.toString(reportPort));
			report.put("downstream_bw", Double.toString(s2cspd));
			report.put("upstream_bw", Double.toString(sc2sspd));
			
			report.put("sack_enabled",Integer.toString(SACKEnabled));
			report.put("nagle_enabled", Integer.toString(NagleEnabled));
			report.put("ECN_enabled",Integer.toString(ECNEnabled));
			report.put("time_stamping_enabled", Integer.toString(TimestampsEnabled));
			...
			if ( getParameter("disableReport") != null ) submit_report = false;
			
			//if the report is to be submitted, do some tests
			if(submit_report) {
				boolean show_popup = false;//by default, do not disturb the user 
				String popup_string = "";
				boolean ip_out_of_country = false;

				//check down and up speeds and warn accordingly
				if((getParameter("max_down") != null) && (getParameter("max_up") != null) ) {
					boolean speed_exceeded = false;
					String message = "";
					double max_down = Double.parseDouble(getParameter("max_down"));
					double max_up = Double.parseDouble(getParameter("max_up"));
					DecimalFormat df1 = new DecimalFormat("#.#");
					DecimalFormat df2 = new DecimalFormat("###");
					if (s2cspd>max_down) {
						speed_exceeded = true;
						show_popup = true;
						submit_report=false;//do not submit the report
						System.err.println(s2cspd+">"+max_down);
	
						String str = (max_down>=1.0)? df1.format(max_down)+"Mbps" : df2.format(max_down*1000)+"Kbps";
						//message += res.getString("speed_exceeded3")+String.format(" %.0fMbps\n",max_down);
						message += res.getString("speed_exceeded3")+" "+str+"\n";
					}
					if (sc2sspd>max_up ) {
						speed_exceeded = true;
						show_popup = true;
						submit_report=false; //do not submit the report
						String str = (max_up>=1.0)? df1.format(max_up)+"Mbps" : df2.format(max_up*1000)+"Kbps";
						//message += res.getString("speed_exceeded4")+String.format(" %.0fMbps\n",max_up);
						message += res.getString("speed_exceeded4")+" "+str;
					}
					if(speed_exceeded) 
						popup_string = res.getString("speed_exceeded1")+"\n"+res.getString("speed_exceeded2")+"\n"+message;
				}
				
				//
				// WARNING **** WARNING **** WARNING
				// UGLY HACK ** UGLY HACK ** UGLY HACK
				// Try to ascertain whether the measurement server IP is in Greece (starts with 83.212.)
				// Most obviously, this is going to break, badly, in the (not so distant) future. Beware!			
				// I wash my hands of this 
				//
				String server_ip_address = "unknown";
				try {
					server_ip_address = (String)InetAddress.getByName(host).getHostAddress();
				} catch(Exception e) { 
					System.err.println("cannot resolve "+host+" to an IP. Error was:"+e.getMessage());
					show_popup=false;
					submit_report=false;
				}
				...
				if(!server_ip_address.startsWith("83.212.")) {
					...
					show_popup = true;
					submit_report = false;//do not submit the report 
					popup_string = (popup_string.length() == 0)? 
							res.getString("server_ip_out_of_country")
							:
							popup_string + "\n" + res.getString("server_ip_out_of_country2")+"\n";
					;
				}
				if (show_popup) popup(JOptionPane.ERROR_MESSAGE,"Maximum Possible Speed exceeded",popup_string);
			}
...
// 					this may become the new submit method in the future
//					String rep_str = "http://whatever/"+"?q="+URLEncoder.encode(json.toString(),"UTF-8");
//					System.err.println(rep_str);
//					URL report_url = new URL(rep_str);
//					URLConnection url_connection = report_url.openConnection();
...
//			        in.close();
					...
					System.out.println("Connecting to " + reportHost + ":" + reportPort
							+ " to submit my results\n");
					Socket reportSocket = new Socket(reportHost, reportPort);
					//don't wait endlessly, just 6 seconds
					reportSocket.setSoTimeout(6000);
					System.err.println("report socket timeout set to "+reportSocket.getSoTimeout()+" milliseconds");
					BufferedReader inResponse = new BufferedReader(new InputStreamReader(reportSocket.getInputStream()));
					String returned = inResponse.readLine();
					System.out.println("Got response from report server: "+returned+"\n");//returned should be 'ok'
					DataOutputStream reportOut = new DataOutputStream(reportSocket
							.getOutputStream());
					reportOut.writeBytes(json.toString());
					
					reportSocket.close();
					System.out.println("results submitted");
				} catch (Exception e) {
					System.err.println("Sorry! Cannot submit results back to "
							+ reportHost + "\nError is:" + e.getMessage());
					popup(JOptionPane.ERROR_MESSAGE,"Report Error",res.getString("socket_dialog1")+"\n"+res.getString("socket_dialog2")+" "+reportPort);
	
				}//try-catch
			}//if submit_report
		}//if ! failed
		else {
			popup(JOptionPane.ERROR_MESSAGE,"Measurement Error",res.getString("measurement_error1")+"\n"+pub_errmsg);
		}
  ...
  public void popup(int type,String title,String message) {
    ...
    if ((tests & TEST_S2C) == TEST_S2C) {
      ...
      if (s2cspd < 1.0) {
		...
		// Expose download speed to JavaScript clients
           pub_s2cspd = s2cspd;
      dnlLbl.setText(String.format(padding+"%.3f Mbps",s2cspd));
           pub_status = "done";
  ...
  public boolean test_meta(Protocol ctl, String application) throws IOException
  {
    ...
    if ((tests & TEST_META) == TEST_META) {
      ...
      System.err.println(
    		  META_CLIENT_OS + ":" + System.getProperty("os.name") 
    		  + " "+META_BROWSER_OS + ":" + UserAgentTools.getBrowser(getUserAgent())[2]
    		  + " "+META_CLIENT_KERNEL_VERSION + ":" + System.getProperty("os.version")
    		  + " "+META_CLIENT_VERSION + ":" + VERSION 
    		  + " "+META_CLIENT_APPLICATION + ":" + application);
...
//				+ (CurrentMSS*8/1024) + "kbits, RTT = " + prtdbl(avgrtt) + "msec, " + "and loss = " + loss + "\n");
	...
	*/
	...
	public void middleboxResults(String tmpstr2) {
		...
		if (mss == 1456) {
			...
			report.put("packet_size_preserved", "1");
		}
		else {
			statistics.append(messages.getString("middleboxModifyingMss") + "\n");
			report.put("packet_size_preserved", "0");
		}
    ...
    try {
      ...
      try {
		...
		report.put("ssip",ssip);
		report.put("scip", scip);
		report.put("ccip", ccip);
  ...
  public static void main(String[] args)
  {
    ...
    if (args.length != 5) {
      System.out.println("Usage: java -jar Tcpbw100.jar " + "HOST ReportHost ReportPort UserID MeasurementID ConnectionId");
      System.exit(0);
    }
